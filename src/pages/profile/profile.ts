...
constructor(private store: Store<State>,
  ...
  ionViewDidLoad() {
   ...
   this.facebook.api('/me?fields=id,name,first_name,picture.width(320).height(120).as(picture_large)',[])
    .then(profile => {
      this.userData['name'] = profile['name'];
      this.userData['first_name'] = profile['first_name'];
      this.userData['picture'] = profile['picture_large']['data']['url'];
    }, (err) => {
